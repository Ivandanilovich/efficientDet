...
from anchors import anchors_for_shape, anchor_targets_bbox, AnchorParameters
...
class PascalVocGenerator(Generator):
    ...
    def load_annotations(self, image_index):
        ...
        try:
            tree = ET.parse(os.path.join(self.data_dir, 'annots', filename))
...
if __name__ == '__main__':
    ...
    for batch_inputs, batch_targets in train_generator:
        ...
        x2 = boxes[:, 2] + (deltas[:, 0] * std_[2] + mean_[2]) * width
        ...
        for x1_, y1_, x2_, y2_, class_id in zip(x1, y1, x2, y2, class_ids):
            ...
            class_name = 'b' # train_generator.labels[class_id]
            label = class_name
